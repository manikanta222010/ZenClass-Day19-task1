...
<section class="container">
    ...
    <div class="paginationContainer" id="pagination">
    </div>
</section>
`
...
let current_page = 1
let rows = 10
let page_count = Math.ceil(paginationData.length / rows);
function display(items, blocksWrapper, rows_per_page, page) {
    ...
    for (let i = 0; i < paginatedItems.length; i++) {
        ...
        let email = paginatedItems[i].email;

        let block = document.createElement('div');
        block.classList.add("block")
        block.classList.add("col-12", "col-md-5")
        block.innerHTML += `
        <p><b>ID: </b>${id}</p>
        <p><b>Name: </b>${name}</p>
        <p><b>Email:</b> ${email}</p>
...
function SetupPagination(items, blocksWrapper) {
    blocksWrapper.innerHTML = ``;

    for (let i = 0; i <= page_count + 1; i++) {
        if (i === 0) {
            let btn = PreviousButton(i, items);
            blocksWrapper.appendChild(btn);
        }
        else if (i === page_count + 1) {
            let btn = NextButton(i, items);
            blocksWrapper.appendChild(btn);
        }
        else {
...
function PreviousButton(page, items) {
    ...
    button.addEventListener('click', function () {
        if (current_page > 1) {
            ...
            display(items, blocks, rows, current_page);

            let current_btn = document.querySelector('#pagination button.active');
            let next_btn = document.querySelector('#pagination button.active').previousElementSibling;
...
function NextButton(page, items) {
    let button = document.createElement('button');
    button.classList.add('btn-grad')
    ...
    button.addEventListener('click', function () {
        if (current_page < page_count) {
            ...
            let current_btn = document.querySelector('#pagination button.active');
            let next_btn = document.querySelector('#pagination button.active').nextElementSibling;
            current_btn.classList.remove('active');
            next_btn.classList.add('active');
        }
    });
...
display(paginationData, blocks, rows, current_page);
SetupPagination(paginationData, pagination_elements);